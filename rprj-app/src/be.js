...
class BackEndProxy {
    ...
    // }
    searchByName(name, uselike, tablenames, ignore_deleted, a_callback) {
        this.con.searchByName(name, uselike, tablenames, ignore_deleted, a_callback);
    }
    ...
    getNewDBEInstance(aclassname, father_id, a_callback) {
        if(aclassname===null || aclassname===undefined || aclassname.length===0) return;
        this.con.getNewDBEInstance(aclassname, father_id, a_callback);
    }
    ...
    insert(dbe, a_callback) {
        this.dbmgr.Insert(dbe, a_callback)
    }
    ...
    return (...args) => {
        ...
        // ELSE call the function cb, aith my_cb as a callback
            var my_cb = (jsonObj, myobj) => {
                ...
                // console.log("simpleCache.my_cb: "+key+" => jsonValue="+jsonValue)
