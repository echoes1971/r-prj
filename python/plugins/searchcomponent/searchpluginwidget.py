...
from rprj.formulator.searchwidget import SearchWidget
...
class SearchPluginWidget(QtGui.QWidget):
	def __init__(self,parent,objectName,filterFormName="FTodo"): # FObject
		QtGui.QWidget.__init__(self,parent)
		self.filterFormName = filterFormName
		w = self
		...
		# Add widgets here
		self.searchWidget = None #SearchWidget(w,"searchWidget_%s"%objectName,filterForm,True)
		#self.webAddress.setObjectName("webAddress_%s"%objectName)
		#self.gridLayout.addWidget(self.searchWidget, 0, 2, 1, 1)
		# ...
		# Slots
		self.bind()
		# Logic
...
# ############################## PROPERTIES ###################################
	...
	def setFormFactory(self,formFactory):
		...
		if self.searchWidget is None:
			self.filterForm = self.formFactory.getInstance(self.filterFormName,dbmgr=self.server)
			self.searchWidget = SearchWidget(self,"searchWidget_%s"%self.objectName,self.filterForm,True)
			self.searchWidget.setServer(self.server)
			self.searchWidget.setFormFactory(self.formFactory)
			self.gridLayout.addWidget(self.searchWidget, 0, 2, 1, 1)
		print "SearchPluginWidget.setFormFactory: TODO"
	def getName(self):
