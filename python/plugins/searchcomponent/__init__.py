...
from rprj.apps.qtcomponent import *
from rprj.formulator import apps_rc
import searchpluginwidget

class SearchComponent(qtComponent):
	"""Search Component"""
	def __init__(self, config, parent=None):
		qtComponent.__init__(self, config, parent)
		self._objectNamePrefix="rprj_search"
		self.widget = None
		self.server = None
...
# ############################## DESCRIPTION ###################################
	def getIcon(self):
		...
		return QtGui.QIcon("%s%skryptonite-32x32.png" % (os.path.dirname(__file__),os.path.sep)) #":/rPrj/icons/linneighborhood.png")
	def getName(self):
		return "Search"
...
# ############################## LOGIC ###################################
	def connectToServer(self, server,  formFactory):
		...
		try:
			...
			widget = searchpluginwidget.SearchPluginWidget(self.widget, "SearchPluginWidget")
			print "SamplePluginComponent.connectToServer: TODO"
