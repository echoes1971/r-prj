...
def dbeToRaw(dbe):
    ...
    for n in dbe.getNames():
        ...
        if isinstance(tmpvalue, datetime.datetime) or isinstance(tmpvalue, datetime.date) or \
                isinstance(tmpvalue, datetime.time):
            # or isinstance(tmpvalue,Binary):
            ret[n] = "%s" % tmpvalue
        else:
            ret[n] = tmpvalue
...
def dataToRaw(d):
    if isinstance(type(d), types.InstanceType):
        return dbeToRaw(d)
        ...
        if dbmgr.isDateTime(v): #rra.dblayer.isDateTime(v):
            ...
            try:
                v2 = datetime.datetime(
                    int(v[:4]), int(v[5:7]), int(v[8:10]), int(v[11:13]), int(v[14:16]), int(v[17:19]))
            except Exception, e:
                try:
...
#                myurl = "%s/xmlrpc_server.php" % ( myurl )
    ...
    def login(self, *args):
        ...
        try:
            ...
            print myerr
            print "".join(traceback.format_tb(sys.exc_info()[2]))
        if self.dbmgr.user is not None:
...
if __name__ == '__main__':
    ...
    if 'SESSION_MANAGER' in os.environ and os.environ['SESSION_MANAGER'].find('roberto-ra') >= 0:
        # I'm at work :-)
        print "ping:", s.ping()
