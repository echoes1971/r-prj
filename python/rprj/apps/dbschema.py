...
from rprj.dblayer import ForeignKey, DBEntity, DBAssociation, DBEFactory, DBLayerException, DBMgr
...
class DBEUser(DBEntity):
    ...
    def _createGroup(self, dbmgr):
        if not self.getValue('group_id') is None:
            return
        dbe = DBEGroup()
...
class DBEObject(DBEntity):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "objects", "id")]
    ...
    def isDeleted(self):
        ...
        if myvalue is not None and not type(myvalue) == str:
...
class ObjectMgr(DBMgr):
    ...
    def select(self, tablename, searchString):
        ...
        if self._verbose:
            print "ObjectMgr.select: start."
        tmp = DBMgr.select(self, tablename, searchString)
        myuser = self.getDBEUser()
        if myuser is not None and myuser.isRoot():
            return tmp
        ret = []
        for obj in tmp:
            if isinstance(obj, DBEObject):
                if myuser is not None and obj.getValue('creator') == myuser.getValue('id'):
                    ret.append(obj)
                    continue
                if obj.isDeleted():
                    continue
    ...
    def insert(self, dbe, cleanKeysIfError=True):
        ...
        if isinstance(dbe, DBEObject):
            ...
            myuser = self.getDBEUser()
            if myuser is not None and dbe.getValue('creator') == myuser.getValue('id'):
                have_permission = True
    ...
    def delete(self, dbe):
        ...
        if self._verbose:
            print "ObjectMgr.delete: query=%s" % query
        try:
        ...
        for tablename, clazz in tipi.items():
            ...
            q.append(
                DBMgr._buildSelectString(self, mydbe, uselike, case_sensitive).replace(
                    "select * ",
                    "select '%s' as classname,id,owner,group_id,permissions,creator,creation_date,last_modify,"
                    "last_modify_date,father_id,name,description " % (mydbe.getTypeName())
                )
            )
        searchString = " union ".join(q)
        if self._verbose:
            print "ObjectMgr._buildSelectString: searchString=%s" % (searchString)
        return searchString
    ...
    def search(self, dbe, uselike=1, orderby=None, ignore_deleted=True, full_object=True):
        ...
        if self._verbose:
            print "ObjectMgr.search: start."
            print "ObjectMgr.search: dbe.getTypeName()=%s." % (dbe.getTypeName())
        if dbe.getTypeName() != "DBEObject":
            ...
            if self._verbose:
                print "ObjectMgr.search: quick end; ret = %s" % (len(ret))
            return ret
        if ignore_deleted:
            if self._connProvider.getDBType() == "POSTGRESQL":
                dbe.setValue('deleted_date', 'null')
        ...
        for _obj in tmp:
            ...
            if self._verbose:
                print "ObjectMgr.search: lista=%s" % (len(lista))
            if len(lista) != 1:
                continue
            ret.append(lista[0])
        if self._verbose:
            print "ObjectMgr.search: end."
        return ret
    ...
    def objectById(self, myid, ignore_deleted=True):
        ...
        for tablename, clazz in tipi.items():
            ...
            if not isinstance(mydbe, DBEObject) or isinstance(mydbe, DBAssociation):
                # or mydbe.getTypeName=="DBEObject":
                continue
            tmp_q = "select '%s' as classname,id,owner,group_id,permissions,creator,creation_date,last_modify," \
                "last_modify_date,father_id,name,description" \
                " from %s where id='%s'" % (mydbe.getTypeName(), self.buildTableName(mydbe), DBEntity.hex2uuid(myid))
            if ignore_deleted:
                if self._connProvider.getDBType() == "POSTGRESQL":
        ...
        searchString = " union ".join(q)
        if self._verbose:
            print "ObjectMgr.objectById: searchString=%s" % (searchString)
        lista = self.select("objects", searchString)
        if len(lista) == 1:
            return lista[0]
        if self._verbose:
            print "ObjectMgr.objectById: lista=%s" % (len(lista))
        return None
    ...
    def fullObjectById(self, myid, ignore_deleted=True):
        ...
        if self._verbose:
            print "ObjectMgr.fullObjectById: lista=%s" % (len(lista))
        if len(lista) == 1:
            return lista[0]
        return None
...
class DBECompany(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "companies", "id"),
            ForeignKey("fk_countrylist_id", "countrylist", "id")]
...
class DBEPeople(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "people", "id"),
            ForeignKey("fk_countrylist_id", "countrylist", "id"), ForeignKey("fk_companies_id", "companies", "id"),
            ForeignKey("fk_users_id", "users", "id")]
...
class DBEEvent(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "events", "id"),
            ForeignKey("fk_obj_id", "companies", "id"), ForeignKey("fk_obj_id", "folders", "id"),
            ForeignKey("fk_obj_id", "people", "id"), ForeignKey("fk_obj_id", "projects", "id")]
...
class DBEFile(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "files", "id"),
            ForeignKey("father_id", "folders", "id"), ForeignKey("fk_obj_id", "pages", "id"),
            ForeignKey("father_id", "pages", "id"), ForeignKey("fk_obj_id", "news", "id"),
            ForeignKey("father_id", "news", "id")]
    ...
    def _before_insert(self, dbmgr=None):
        ...
        if self.getValue('filename') > '':
            ...
            if not os.path.exists(dest_dir):
                os.mkdir(dest_dir)
            nuovo_filename = self.generaFilename(self.getValue('id'), self.getValue('filename').split(os.path.sep)[-1])
    ...
    def _before_update(self, dbmgr=None):
        ...
        if father_id > '':
            query = "select fk_obj_id from %s  where id='%s'" % (
                dbmgr.buildTableName(self), self.getValue('father_id'))
        ...
        if self.getValue('filename') > '':
            ...
            from_filename = self.generaFilename(self.getValue('id'), self.getValue('filename').split(os.path.sep)[-1])
            if from_path > '':
...
class DBEFolder(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "folders", "id"),
            ForeignKey("fk_obj_id", "companies", "id"), ForeignKey("fk_obj_id", "people", "id"),
            ForeignKey("fk_obj_id", "projects", "id")]
    ...
    def _before_update(self, dbmgr=None):
        ...
        if father_id > '':
            query = "select fk_obj_id from %s where id='%s'" % (
                dbmgr.buildTableName(self), ("%-16s" % self.getValue('father_id')).replace(' ', '\0'))
            tmp = dbmgr.select(self.getTableName(), query)
            if len(tmp) == 1:
                self.setValue('fk_obj_id', tmp[0].getValue('fk_obj_id'))
...
class DBELink(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "links", "id"),
            ForeignKey("fk_obj_id", "companies", "id"), ForeignKey("fk_obj_id", "folders", "id"),
            ForeignKey("fk_obj_id", "people", "id"), ForeignKey("fk_obj_id", "projects", "id"),
            ForeignKey("fk_obj_id", "pages", "id"), ForeignKey("father_id", "pages", "id"),
            ForeignKey("fk_obj_id", "news", "id"), ForeignKey("father_id", "news", "id")]
...
class DBENote(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "notes", "id"),
            ForeignKey("fk_obj_id", "companies", "id"), ForeignKey("fk_obj_id", "folders", "id"),
            ForeignKey("fk_obj_id", "people", "id"), ForeignKey("fk_obj_id", "projects", "id")]
...
class DBEPage(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "pages", "id"),
            ForeignKey("fk_obj_id", "companies", "id"), ForeignKey("fk_obj_id", "folders", "id"),
            ForeignKey("fk_obj_id", "people", "id"), ForeignKey("fk_obj_id", "projects", "id")]
...
class DBENews(DBEPage):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "news", "id"),
            ForeignKey("fk_obj_id", "companies", "id"), ForeignKey("fk_obj_id", "folders", "id"),
            ForeignKey("fk_obj_id", "people", "id"), ForeignKey("fk_obj_id", "projects", "id")]
...
class DBEBanned(DBEObject):
    ...
    def __init__(self, tablename=None, names=None, values=None, attrs=None, keys={'id': 'number'}):
        ...
        if len(DBEBanned.__columns.keys()) == 0:
            ...
            DBEBanned.__columns['give_reason'] = [
                "varchar(255)", "default 'Your IP has been blocked.<br/>See http://adf.ly/XdZw for more details.'"]
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "banned", "id")]
...
class DBEMail(DBEFile):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "mail", "id"),
            ForeignKey("father_id", "folders", "id"), ForeignKey("fk_obj_id", "pages", "id"),
            ForeignKey("father_id", "pages", "id"), ForeignKey("fk_obj_id", "news", "id"),
            ForeignKey("father_id", "news", "id")]
    ...
    def parseEmail(self, filename):
        ...
        if 'Date' in msg:
            try:
                ...
                self.setValue('msgdate', datetime.datetime(
                    mytuple[0], mytuple[1], mytuple[2], mytuple[3], mytuple[4], mytuple[5], mytuple[6]))
            except Exception:
                pass
        msgbody = ''
...
class DBEProject(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "projects", "id")]
...
class DBEProjectCompanyRole(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "projects_companies_roles", "id")]
...
class DBEProjectCompany(DBEntity):
    ...
    def getFK(self):
        return [
            ForeignKey("project_id", "projects", "id"), ForeignKey("company_id", "companies", "id"),
            ForeignKey("projects_companies_role_id", "projects_companies_roles", "id")]
...
class DBEProjectPeople(DBEntity):
    ...
    def getFK(self):
        return [
            ForeignKey("project_id", "projects", "id"), ForeignKey("people_id", "people", "id"),
            ForeignKey("projects_people_role_id", "projects_people_roles", "id")]
...
class DBEProjectProjectRole(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "projects_projects_roles", "id")]
...
class DBEProjectProject(DBEntity):
    ...
    def getFK(self):
        return [
            ForeignKey("project_id", "projects", "id"), ForeignKey("project2_id", "projects", "id"),
            ForeignKey("projects_projects_role_id", "projects_projects_roles", "id")]
...
class DBETimetrack(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "timetracks", "id"),
            ForeignKey("fk_obj_id", "projects", "id"), ForeignKey("fk_obj_id", "folders", "id"),
            ForeignKey("fk_obj_id", "todo", "id"), ForeignKey("fk_progetto", "projects", "id")]
...
class DBETodo(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "todo", "id"),
            ForeignKey("fk_segnalato_da", "people", "id"), ForeignKey("fk_cliente", "companies", "id"),
            ForeignKey("fk_progetto", "projects", "id"), ForeignKey("father_id", "folders", "id"),
            ForeignKey("father_id", "todo", "id"), ForeignKey("fk_tipo", "todo_tipo", "id")]
...
class DBETodoTipo(DBEObject):
    ...
    def getFK(self):
        return [
            ForeignKey("owner", "users", "id"), ForeignKey("group_id", "groups", "id"),
            ForeignKey("creator", "users", "id"), ForeignKey("last_modify", "users", "id"),
            ForeignKey("deleted_by", "users", "id"), ForeignKey("father_id", "todo_tipo", "id")]
