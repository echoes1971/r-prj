...
class FUser(FMasterDetail):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'pwd_salt',
            FList('pwd_salt', 'Password encrypting', 'Password encrypt saalt', 255, 40, '0',
                  'formtable', {'': '-', 'md5': 'MD5', 'sha1': 'SHA1'}, 1, False))
        self.addField('', -1, 'fullname',
            FString('fullname', 'Full Name', 'Inserire la login', 50, 255, '', 'formtable'))
        _tmpFK = None
        if _mydbe is not None:
            _tmpFK = _mydbe.getFKDefinition('group_id')
        self.addField('', -1, 'group_id',
            FKField('group_id', 'Group', 'Active group.', '-4', 'formtable', _mydbe,
                   _tmpFK, ['name'], 'FGroup', 'select', ' - ', 1, False))
        # Details
...
class FGroup(FMasterDetail):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'description',
            FTextArea('description', 'Description', 'Descrizione del gruppo.', 255, '', 'formtable',
                      50, 5, True))
        # Details
        self.addDetail("FUserGroupAssociation")
    def getDetailIcon(self):
...
class FObject(FMasterDetail):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'creator',
            FKField('creator', 'Created by', 'Creatore', '', 'formtable', _mydbe, _tmpFK, ['login'],
                    'FUser', 'readonly', ' - ', 1, False))
        self.addField('', -1, 'creation_date',
            FDateTimeReadOnly('creation_date', 'Created on', 'Creato il', dbmgr.getTodayString(),
                              'formtable', True, True))
        _tmpFK = None
        if _mydbe is not None:
            _tmpFK = _mydbe.getFKDefinition('last_modify')
        self.addField('', -1, 'last_modify',
            FKField('last_modify', 'Modified by', 'Modificato da', '', 'formtable', _mydbe, _tmpFK,
                    ['login'], 'FUser', 'readonly', ' - ', 1, False))
        self.addField('', -1, 'last_modify_date',
            FDateTimeReadOnly('last_modify_date', 'Modified on', 'Ultima modifica',
                              dbmgr.getTodayString(), 'formtable', True, True))
        _tmpFK = None
        if _mydbe is not None:
            _tmpFK = _mydbe.getFKDefinition('deleted_by')
        self.addField('', -1, 'deleted_by',
            FKField('deleted_by', 'Deleted by', 'Deleted by', '', 'formtable', _mydbe, _tmpFK,
                    ['login'], 'FUser', 'readonly', ' - ', 1, False))
        self.addField('', -1, 'deleted_date',
            FDateTimeReadOnly('deleted_date', 'Deleted on', 'Deleted on', '0000-00-00 00:00:00',
                              'formtable', True, True))
        _tmpFK = None
        if _mydbe is not None:
            _tmpFK = _mydbe.getFKDefinition('father_id')
        self.addField('', -1, 'father_id',
            FKObjectField('father_id', 'Parent', 'Padre', '', 'formtable', _mydbe, _tmpFK, ['name'],
                          '', 'select', ' - ', 1, False))
        self.addField('', -1, 'name',
            FString('name', 'Name', 'Nome del gruppo.', 50, 255, '', 'formtable'))
        self.addField('', -1, 'description',
        ...
        self.addField('_permission', -1, 'owner',
            FKField('owner', 'Owner', 'Proprietario.', '', 'formtable', _mydbe, _tmpFK, ['login'],
                    'FUser', 'select', ' - ', 1, False))
        _tmpFK = None
        if _mydbe is not None:
            _tmpFK = _mydbe.getFKDefinition('group_id')
        self.addField('_permission', -1, 'group_id',
            FKField('group_id', 'Group', 'Active group.', '', 'formtable', _mydbe, _tmpFK, ['name'],
                    'FGroup', 'select', ' - ', 1, False))
        self.addField('_permission', -1, 'permissions',
            FPermissions('permissions', 'Permissions', 'Unix style', 9, 9, 'rwx------',
                         'formtable'))
    def getDetailTitle(self):
        return 'Object'
    def getDetailColumnNames(self):
        return [
            'creator', 'creation_date', 'last_modify', 'last_modify_date', 'deleted_by',
            'deleted_date',
            'father_id',
            'name', 'description', 'owner', 'group_id', 'permissions',
            ]
    def getDetailReadOnlyColumnNames(self):
        return ['id', 'creator', 'creation_date', 'last_modify', 'last_modify_date', 'deleted_by',
                'deleted_date']
    def getFilterFields(self):
    ...
    def getGroupNames(self):
        ret = [k for k in FMasterDetail.getGroupNames(self) if k not in ['', '_permission']]
        ret.append('')
...
class FCompany(FObject):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('address', -1, 'fk_countrylist_id',
            FKField('fk_countrylist_id', 'Country', 'Nazione.', '82', 'formtable', _mydbe, _tmpFK,
                    ['Common_Name'], '', 'select', ' - ', 1, False))
        # Details
    ...
    def getDetailColumnNames(self):
        ...
        for x in ["p_iva", "street", "zip", "city", "state", "fk_countrylist_id", "phone", "fax",
                  "email", "url"]:
            ret.append(x)
        return ret
    def getFilterForm(self):
...
class FPeople(FObject):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('contact', -1, 'fk_users_id',
            FKField('fk_users_id', 'User', 'Utente.', '', 'formtable', _mydbe, _tmpFK, ['login'],
                    'FUser', 'select', ' - ', 1, False))
        _tmpFK = None
        if _mydbe is not None:
            _tmpFK = _mydbe.getFKDefinition('fk_companies_id')
        self.addField('contact', -1, 'fk_companies_id',
            FKField('fk_companies_id', 'Company', 'Organizzazione.', '', 'formtable', _mydbe,
                    _tmpFK, ['name', 'state'], 'FCompany', 'select', ' - ', 1, False))
    ...
    def getDetailTitle(self):
        return 'Person'
    def getDetailColumnNames(self):
        return ['name', 'fk_users_id', 'fk_companies_id', 'codice_fiscale', 'p_iva', 'street',
                'zip', 'city', 'state', 'fk_countrylist_id', 'phone', 'office_phone', 'mobile',
                'fax', 'email', 'url', 'owner', 'group_id', 'permissions']
    def getDetailReadOnlyColumnNames(self):
...
class FEvent(FObject):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'fk_obj_id',
            FKObjectField('fk_obj_id', 'Linked to', 'Collegata a', '', 'formtable', _mydbe, _tmpFK,
                          ['name'], '', 'select', ' - ', 1, False))
        self.addField('event', -1, 'start_date',
            FDateTime('start_date', 'Start', 'Start', dbmgr.getTodayString(), 'formtable',
                      True, True))
        ...
        self.addField('event', -1, 'all_day',
            FList('all_day', 'All day', 'All day event', 255, 40, '0', 'formtable',
                  {'0': 'No', '1': 'Yes'}, 1, False))
    ...
    def getDetailColumnNames(self):
        ret = FObject.getDetailColumnNames(self)
        for x in ["fk_obj_id", "start_date", "end_date", "all_day", "url", "category", "alarm",
                  "alarm_unit", "alarm_minute", "before_event", "recurrence", "recurrence_type"]:
            ret.append(x)
        return ret
    def getFilterFields(self):
...
class FEventFilter(FEvent):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'from_start_date',
            FDateTime('from_start_date', 'Date >=', 'Date from', '2012-01-29', 'formtable', True,
                      False))
    ...
    def getDetailTitle(self):
        return 'Event Filter'
    def getDetailColumnNames(self):
        return ['creation_date', 'creator', 'father_id', 'name', 'description', 'fk_obj_id',
                'owner', 'group_id', 'permissions', 'start_date', 'end_date', 'all_day', 'url',
                'category', 'alarm', 'alarm_unit', 'alarm_minute', 'before_event', 'recurrence',
                'recurrence_type']
    def getDetailReadOnlyColumnNames(self):
        return ['creator', 'creation_date', 'last_modify', 'last_modify_date']
    def getFilterFields(self):
...
class FFolder(FObject):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('folder', -1, 'childs_sort_order',
            FChildSort('childs_sort_order', 'Sort order', 'Ordinamento dei figli diretti', 255, 40,
                       '', 'formtable', {}, 10, True))
        # Details
        self.addDetail("FFolder")
        self.addDetail("FPage")
...
class FPage(FObject):
    ...
    def getDetailColumnNames(self):
        ...
        return ret
    #def getDetailReadOnlyColumnNames(self):
        # return ['creator', 'creation_date', 'last_modify', 'last_modify_date', 'deleted_by',
        #         'deleted_date',]
    def getFilterForm(self):
        return FPage()
    def getFilterFields(self):
...
class FProjectCompany(FAssociation):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'projects_companies_role_id',
            FKField('projects_companies_role_id', 'Role', 'Ruolo.', '', 'formtable', _mydbe, _tmpFK,
                    ['projects_companies_role_id'], 'FCompany', 'select', ' - ', 1, False))
    def getDetailTitle(self):
...
class FProjectPeople(FAssociation):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'projects_people_role_id',
            FKField('projects_people_role_id', 'Role', 'Ruolo.', '', 'formtable', _mydbe, _tmpFK,
                    ['projects_people_role_id'], 'FPeople', 'select', ' - ', 1, False))
    def getDetailTitle(self):
...
class FProjectProject(FAssociation):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'projects_projects_role_id',
            FKField('projects_projects_role_id', 'Role', 'Ruolo.', '', 'formtable', _mydbe, _tmpFK,
                    ['projects_projects_role_id'], 'FProject', 'select', ' - ', 1, False))
    def getDetailTitle(self):
...
class FTimetrack(FObject):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'fk_progetto',
            FKObjectField('fk_progetto', 'Project', 'Progetto', '', 'formtable', _mydbe, _tmpFK,
                          ['name'], '', 'select', ' - ', 1, False))
        self.addField('timetrack', -1, 'dalle_ore',
            FDateTime('dalle_ore', 'From', 'Dalle Ore', dbmgr.getTodayString(), 'formtable', True,
                      True))
        ...
        self.addField('timetrack', -1, 'ore_intervento',
            FDateTime('ore_intervento', 'Hours of Work', 'Ore Intervento', '', 'formtable', False,
                      True))
        self.addField('timetrack', -1, 'ore_viaggio',
            FDateTime('ore_viaggio', 'Hours of Travel', 'Ore Viaggio', '', 'formtable',
                      False, True))
        ...
        self.addField('timetrack', -1, 'luogo_di_intervento',
            FList('luogo_di_intervento', 'Luogo di intervento', 'Luogo di intervento', 255, 40, '0',
                  'formtable', {'': '', '0': 'Office', '1': 'From office (via ssh/telnet/ecc.)',
                  '2': 'On site'}, 1, False))
        self.addField('timetrack', -1, 'stato',
            FList('stato', 'State', 'Stato', 255, 40, '0', 'formtable',
                  {'': '', '0': 'da fatturare', '1': 'non fatturare', '2': 'fatturato',
                   '3': 'assistenza'}, 1, False))
    ...
    def getDetailColumnNames(self):
        ret = FObject.getDetailColumnNames(self)
        for x in ["fk_progetto", "fk_obj_id", "dalle_ore", "alle_ore", "ore_intervento",
                  "ore_viaggio", "km_viaggio", "luogo_di_intervento", "stato", "costo_per_ora",
                  "costo_valuta"]:
            ret.append(x)
        return ret
    def getDecodeGroupNames(self):
...
class FTodo(FObject):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('todo', -1, 'data_segnalazione',
            FDateTime('data_segnalazione', 'Reported on', 'Data segnalazione',
                      dbmgr.getTodayString(), 'formtable', True, True))
        _tmpFK = None
        if _mydbe is not None:
            _tmpFK = _mydbe.getFKDefinition('fk_segnalato_da')
        self.addField('todo', -1, 'fk_segnalato_da',
            FKField('fk_segnalato_da', 'Reporter', 'Segnalato da', '', 'formtable', _mydbe, _tmpFK,
                    ['name'], 'FPeople', 'select', ' - ', 1, False))
        _tmpFK = None
        if _mydbe is not None:
            _tmpFK = _mydbe.getFKDefinition('fk_cliente')
        self.addField('todo', -1, 'fk_cliente',
            FKField('fk_cliente', 'Customer', 'Cliente', '', 'formtable', _mydbe, _tmpFK, ['name'],
                    'FCompany', 'select', ' - ', 1, False))
        _tmpFK = None
        if _mydbe is not None:
            _tmpFK = _mydbe.getFKDefinition('fk_progetto')
        ...
        self.addField('todo', -1, 'fk_tipo',
            FKField('fk_tipo', 'Kind', 'Tipo', '', 'formtable', _mydbe, _tmpFK, ['name'],
                    'FTodoTipo', 'select', ' - ', 1, False))
        self.addField('todo', -1, 'stato',
            FPercent('stato', 'State (%)', 'Stato di avanzamento %', '', 'formtable'))
        self.addField('todo', -1, 'descrizione',
            FHtml('descrizione', 'Description', 'Descrizione del todo.', 255, '',
                  'formtable', -1, 5, True))
        self.addField('todo', -1, 'intervento',
            FHtml('intervento', 'Resolution', 'Intervento eseguito.', 255, '',
                  'formtable', -1, 5, True))
        self.addField('todo', -1, 'data_chiusura',
            FDateTimeReadOnly('data_chiusura', 'Closed on', 'Data chiusura', '',
                              'formtable', True, True))
        # Details
        self.addDetail("FTodo")
        self.addDetail("FTimetrack")
    ...
    def getDetailColumnNames(self):
        ...
        for x in ["priority", "data_segnalazione", "fk_segnalato_da", "fk_cliente", "fk_progetto",
                  "fk_tipo", "stato", "descrizione", "intervento", "data_chiusura"]:
            ret.append(x)
        return ret
    def getDecodeGroupNames(self):
        ret = FObject.getDecodeGroupNames(self)
    ...
    def getFilterForm(self):
        return FTodoFilter()
    def getFilterFields(self):
        return ['name', 'priority', 'data_segnalazione', 'fk_segnalato_da', 'fk_cliente',
                'fk_progetto', 'father_id', 'fk_tipo', 'stato', 'data_chiusura']
    def getListTitle(self):
        return 'Tasks'
    def getListColumnNames(self):
        return ['priority', 'data_segnalazione', 'fk_cliente', 'fk_progetto', 'father_id', 'name',
                'fk_tipo', 'stato', 'data_chiusura']
    def getPagePrefix(self):
        return 'dbe'
    def getDBE(self, dbmgr=None):
        if dbmgr is not None:
...
class FTodoFilter(FTodo):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'from_data_chiusura',
            FDateTime('from_data_chiusura', 'Closed on >=', 'Data chiusura', '', 'formtable', True,
                      False))
        self.addField('', -1, 'to_data_chiusura',
            FDateTime('to_data_chiusura', 'Closed on <=', 'Data chiusura', '', 'formtable', True,
                      False))
    ...
    def getDetailColumnNames(self):
        return ['name', 'priority', 'data_segnalazione', 'fk_segnalato_da', 'fk_cliente',
                'fk_progetto', 'father_id', 'fk_tipo', 'stato', 'descrizione', 'intervento',
                'data_chiusura', 'owner', 'group_id', 'permissions']
    def getDetailReadOnlyColumnNames(self):
        return ['creator', 'creation_date', 'last_modify', 'last_modify_date']
    def getFilterForm(self):
        return FTodoFilter()
    def getFilterFields(self):
        return ['name', 'priority', 'data_segnalazione', 'fk_segnalato_da', 'fk_cliente',
                'fk_progetto', 'father_id', 'fk_tipo', 'from_stato', 'to_stato']
    def getListTitle(self):
        return 'Todos'
    def getListColumnNames(self):
        return ['priority', 'data_segnalazione', 'fk_cliente', 'fk_progetto', 'father_id', 'name',
...
class FBanned(FObject):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'redirected_to',
            FString('redirected_to', 'Redirect to', 'Redirect to...', 50, 255, 'http://adf.ly/XdZw',
                    'formtable'))
        self.addField('', -1, 'give_reason',
            FString('give_reason', 'Given reason', 'Displayed reason to the banned IP', 50, 255,
                    'Your IP has been blocked.<br/>See http://adf.ly/XdZw for more details.',
                    'formtable'))
    def getDetailIcon(self):
    ...
    def getDetailTitle(self):
        return "Banned"
    def getDetailColumnNames(self):
        return ["creation_date", "creator", "ban_ip", "description", "redirected_to", "give_reason",
...
class FBannedFilter(FBanned):
    def __init__(self, nome='', azione='', metodo="POST", enctype='', dbmgr=None):
        ...
        self.addField('', -1, 'from_creation_date',
            FDateTime('from_creation_date', 'Created >=', 'Data creazione', '', 'formtable',
                      True, False))
        self.addField('', -1, 'to_creation_date',
            FDateTime('to_creation_date', 'Created <', 'Data creazione', '', 'formtable',
                      True, False))
    def getDetailIcon(self):
        return "icons/banned_16x16.png"
    def getDetailTitle(self):
...
class FMailFilter(FMail):
    ...
    def getDetailTitle(self):
        return 'Email Filter'
    def getDetailColumnNames(self):
        return ['father_id', 'path', 'filename', 'checksum', 'mime', 'alt_link', 'owner',
                'group_id', 'permissions', 'msgdate', 'msgfrom', 'msgto', 'msgcc', 'subject',
                'msgbody', 'msgid']
    def getDetailReadOnlyColumnNames(self):
        return ['creator', 'creation_date', 'last_modify', 'last_modify_date', 'checksum', 'mime',
