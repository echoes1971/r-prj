...
class EmailWidget(QtGui.QWidget):
    ...
    def bind(self):
        ...
        QtCore.QObject.connect(
            self.explorer, QtCore.SIGNAL("selected(QStandardItem,PyQt_PyObject)"), self.slotExplorerSelectedItem)
# ############################## PROPERTIES ###################################

    def setContainer(self, c):
        self.container = c
        self.mboxDialog.setDirectory(
            self.getPref(self.container.__class__.__name__, "mboxDirectory", os.path.realpath(".")))
...
# ############################## LOGIC ###################################
    ...
    def checkEmailFolders(self):
        ...
        cerca = self.server.getClazzByTypeName("DBEFolder")(
            attrs={'name': "%s" % EMAIL_FOLDERS_NAME,  'owner': self.server.getDBEUser().getValue('id')})
        ...
        for childname in EMAIL_FOLDERS_DEFAULTS:
            childfolder = self.server.getClazzByTypeName("DBEFolder")(
                attrs={'name': childname, 'father_id': mainfolder.getValue('id')})
