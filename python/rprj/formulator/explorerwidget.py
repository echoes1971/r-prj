...
class ExplorerWidget(QtGui.QWidget):
    ...
    def bind(self):
        ...
        QtCore.QObject.connect(
            self.ui.treeView, QtCore.SIGNAL("doubleClicked(QModelIndex)"), self.slotTreeViewDoubleClick)
        QtCore.QObject.connect(
            self.ui.treeView, QtCore.SIGNAL("customContextMenuRequested(QPoint)"), self.slotTreeViewContextMenu)
...
# ############################## DRAG DROP ###################################
    ...
    def _treeItemAtEventPosition(self, eventPosition):
        ...
        if self.ui.treeView.viewport() == childOver:
            ...
            if item is not None and item.hasChildren() and not self.ui.treeView.isExpanded(item.index()):
    ...
    def dropEvent(self, event):
        ...
        if myformat == 'text/uri-list' or myformat == 'x-special/gnome-icon-list':
            for u in mimeData.urls():
                ...
                icon = QtGui.QIcon()
                icon.addPixmap(QtGui.QPixmap(":/rPrj/%s" % myform.getDetailIcon()), QtGui.QIcon.Normal, QtGui.QIcon.Off)
...
# ############################## PROPERTIES ###################################
    ...
    def setFormFactory(self, formFactory):
        ...
        for _f in formList:
            if _f in ['default', 'FBanned', 'FLog', 'FObject', 'FProjectPeopleRole', 'FProjectCompanyRole',
                      'FTodoTipo', 'FUserGroupAssociation']:
                continue
            if _f.find("Filter") >= 0:
                continue
...
# ############################## UI ###################################
    ...
    def updateTree(self, lista):
        ...
        # 2012.11.11: start.
        self.ids_items = {}
        self.fatherids_items = {}
        rootItem = self.modelTree.invisibleRootItem()
        self.updateItem(rootItem, lista)
        #for l in lista:
    ...
    def updateItem(self, item, lista):
        ...
        for l in lista:
            ...
            # 2012.11.11: start.
            self.ids_items[l.getValue('id')] = child
            if l.getValue('father_id') > '':
                if l.getValue('father_id') not in self.fatherids_items:
                    self.fatherids_items[l.getValue('father_id')] = []
                self.fatherids_items[l.getValue('father_id')].append(child)
            # 1. Check whether I'm son of somebody
            father_id = l.getValue('father_id')
            parentItem = None
            if father_id > '' and father_id in self.ids_items:
                parentItem = self.ids_items[father_id]
            #if father_id>'':
                #parentItem = self._findItemById(father_id)
            if parentItem is not None and not parentItem == rootItem and not parentItem == item:
                parentItem.appendRow(child)
                self.ui.treeView.setExpanded(parentItem.index(), True)
            else:
                item.appendRow(child)
            # 2. Check whether my childs are already here
            if l.getValue('father_id') in self.fatherids_items:
                mychilds = self.fatherids_items[l.getValue('father_id')]
                if len(mychilds) > 0: print "TODO: che fare con i %s childs?" % len(mychilds)
            #item.appendRow( child )
            # 2012.11.11: end.
        self.ui.treeView.setExpanded(item.index(), True)
...
# ############################## SLOTS ###################################
    ...
    def slotTreeViewContextMenu(self, pos):
        ...
        myaction = menu.addAction(icon, QtGui.QApplication.translate("TreeView", "Reload", None,
                                  QtGui.QApplication.UnicodeUTF8), self.slotReload)
        ...
        if myitem == self.modelTree.invisibleRootItem():
            formList = self.formFactory.getAllClassnames()
            for _f in formList:
                if _f in ['default', 'FBanned', 'FEventFilter', 'FLog', 'FObject', 'FProjectPeopleRole',
                          'FProjectCompanyRole', 'FTodoTipo', 'FUserGroupAssociation']:
                    continue
                if _f.find("Filter") >= 0:
                    continue
        ...
        if 'reload' in myactions:
            myactions.pop('reload')
        ...
        if len(myformlist) > 0:
            ...
            for mydetail in myformlist:
                icon = QtGui.QIcon()
                icon.addPixmap(
                    QtGui.QPixmap(":/rPrj/%s" % mydetail.getDetailIcon()), QtGui.QIcon.Normal, QtGui.QIcon.Off)
                myaction = menuAdd.addAction(
                    icon, QtGui.QApplication.translate(
                        "TreeView", "%s" % mydetail.getDetailTitle(), None, QtGui.QApplication.UnicodeUTF8))
        ...
        if len(myactions.keys()) > 0:
            ...
            for k, v in myactions.items():
                if len(myactions[k]) > 2:
                    icon = QtGui.QIcon()
                    icon.addPixmap(
                        QtGui.QPixmap(":/rPrj/icons/%s" % myactions[k][2]), QtGui.QIcon.Normal, QtGui.QIcon.Off)
                    myaction = menuActions.addAction(
                        icon, QtGui.QApplication.translate(
                            "TreeView", myactions[k][0], None, QtGui.QApplication.UnicodeUTF8), self.slotCustomAction)
                else:
                    myaction = menuActions.addAction(
                        QtGui.QApplication.translate("TreeView", myactions[k][0], None, QtGui.QApplication.UnicodeUTF8),
                        self.slotCustomAction)
                myaction.setData("%s" % k)
        menu.addSeparator()
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(_fromUtf8(":/rPrj/icons/editdelete.png")), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        myaction = menu.addAction(icon, QtGui.QApplication.translate("TreeView", "Delete", None,
                                  QtGui.QApplication.UnicodeUTF8), self.slotDelete)
        myaction.setData(None)
    ...
    def slotDelete(self):
        ...
        if mydbe is not None and not mydbe.isDeleted():
