...
# ############################## ... ###################################
    ...
    def Confirm(self, msg):
        ret = QtGui.QMessageBox.question(self.myMainWindow, "Domanda", msg,
...
# ############################## PREFERENCES ###################################
    ...
    def RestoreUIFromConfig(self):
        ...
        serverlist = self.getPref("UILogin", "servers",
                                  ["sqlite:%s" % RPRJ_LOCAL_DB, "http://localhost/rprj/",
                                   "mysql:localhost:rproject:root:", "postgresql:localhost:rproject:roberto:"])
        for s in serverlist:
...
# ############################## LOGIC ###################################
    ...
    def connectToServer(self, url, user, pwd):
        ...
        if self.server.user is not None:
            # New server?
            serverlist = self.getPref(
                "UILogin", "servers", [
                    "http://localhost/rprj/", "sqlite:%s" % RPRJ_LOCAL_DB, "mysql:localhost:rproject:root:",
                    "postgresql:localhost:rproject:roberto:"])
            if url not in serverlist:
